...
表形式のプレーンテキストデータ形式  
各行（「レコード」とも呼ぶ）は独立し、各列（「フィールド」とも呼ぶ）は何らかの区切り文字で区切られる。[タブ区切り（TSV）](https://ja.wikipedia.org/wiki/Tab-Separated_Values)や[カンマ区切り（CSV）](https://ja.wikipedia.org/wiki/Comma-Separated_Values)ファイルなど。  
