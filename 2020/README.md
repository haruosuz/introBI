...
- 補講 [extra classes](#extra-classes) | 8章　R言語入門 | 8. A Rapid Introduction to the R Language
...
- 2020-12-22 No. 12 - 最終回 final class
  ...
  - 2020-12-29 最終課題提出期限 Deadline for [final assignment](https://github.com/haruosuz/introBI/blob/master/2020/CaseStudy.md#assignment-9)
...
`if test -e file` を
`if [ -e file ] ` で代用できる。角括弧`[ ]`の前後に半角スペースが必要。
...
`ls *.fastq`で "Argument list too long" と言われたときの解決策
...
[join - 共通フィールドをもつ２つのファイルを行単位で結合](http://technique.sonots.com/?UNIX%2Fコマンド%2Fテキスト処理%2Fjoin)
	...
	# 基本的な構文: join -1 <file_1_field> -2 <file_2_field> <file_1> <file_2>
...
2番染色体(chr2)と3番染色体(chr3)に領域の長さ（終了位置 - 開始位置）の列を追加する:  
