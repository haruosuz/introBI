...
- 2018-10-02 第02回 バイオインフォマティクスのプロジェクト管理 [Managing a Bioinformatics Project](#2018-10-02)
- 2018-10-09 第03回 Unixシェル [Unix Shell](#2018-10-09)
- 2018-10-16 第04回 バイオインフォマティクス・データ [Bioinformatics Data](#2018-10-16)
- 2018-10-23 第05回 Unixデータツール [Unix Data Tools](#2018-10-23)
- 2018-10-30 第06回 シェルスクリプト [Shell Scripting](#2018-10-30)
...
[Awk](https://ja.wikipedia.org/wiki/AWK)は、入力データをレコード（行）に分割し、各レコードをフィールド（列）に分割する。レコード全体は変数`$0`に格納され、フィールドは`$1, $2, $3, ...`と分割される。
    ...
    awk '{ print $3 "\t" $2 "\t" $1 }' example.bed # reorder columns
