...
#### plasmids
[プラスミド](https://ja.wikipedia.org/wiki/プラスミド)のリスト（*plasmids.txt*）を解析する。
...
2列目（`Kingdom`）の分類群（Archaea, Bacteria, Eukaryota）毎の個数を計算する:  
...
2列目（`Kingdom`）の分類群（Archaea, Bacteria, Eukaryota）毎の百分率を計算する:  
...
プラスミドの宿主細菌の`#Organism/Name`をカウントし、上位12位を表示する:  
