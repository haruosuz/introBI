...
`less README`でファイルを開いて、`/`を押して、文字列`overview`,`eukaryotes`,`prokaryotes`,`viruses`を入力（終了するには、`q`を押す）。

列番号を付けて出力する: 

    cd ~/projects/ncbiGenomeList/data/
    FILE="overview.txt"
    #FILE="eukaryotes.txt" #FILE="prokaryotes.txt"
    #FILE="viruses.txt" #FILE="plasmids.txt"
...
列ラベル（`#`で始まる行）と、[トラフグ属](https://ja.wikipedia.org/wiki/トラフグ属) "Takifugu" にマッチする行を抽出し、`output.txt`ファイルへ出力する:  
...
#### plasmids
[プラスミド](https://ja.wikipedia.org/wiki/プラスミド)のリスト（*plasmids.txt*）を用いて、プラスミドの宿主の分類群（`Kingdom, Group, SubGroup`）を調べる。
...
データの総数を計算する:  
    ...
    grep -v "^#" plasmids.txt | wc -l

2列目（`Kingdom`）のユニークな分類群の個数を計算する:  

    grep -v "^#" plasmids.txt | cut -f2 | sort | uniq -c | awk '{print $1}'

[awkへシェルスクリプトの変数を渡す](https://qiita.com/yamao2253/items/cf69b68447214036e914)

2列目（`Kingdom`）のユニークな分類群の百分率を計算する:  

    TOTAL=$(grep -v "^#" plasmids.txt | wc -l)
    grep -v "^#" plasmids.txt | cut -f2 | sort | uniq -c | awk '{print 100 * $1 / '"$TOTAL"'}'

By 2017-09-08, there were 9939 sequenced plasmids in the NCBI database. Among the 9939 plasmids, 179 (1.8%) from Archaea, 9649 (97.1%) are derived from Bacteria, and 111 (1.1%) from Eukaryota. 

細菌"Bacteria"にマッチする行を抽出し、3,4列目（`Group, SubGroup`）をカウントし、上位10位を表示する:  

    grep "Bacteria" plasmids.txt | cut -f3,4 | sort | uniq -c | sort -nr | head -n 10
...
`viruses.txt`ファイルの5,9列目（`Group`,`Host`）をカウントする:  
...
列ラベル（`#`で始まる行）と、[ゾウリムシ属](https://ja.wikipedia.org/wiki/ゾウリムシ) "Paramecium" にマッチする行の1,7,8列目（`#Organism/Name`,`Size (Mb)`,`GC%`）を抽出する:  

    (grep "^#" eukaryotes.txt; grep "Paramecium" eukaryotes.txt) | cut -f1,7,8
...
以下の通り、4種のゲノムが登録され、ゲノムのサイズは30.48～76.96 Mbの範囲、GC含量は24.4～28.6 %の範囲を示した。 
        ...
        Paramecium tetraurelia strain d4-2	0.984602	27.5
...
#### prokaryotes
原核生物のゲノム・リスト（*prokaryotes.txt*）を用いて、[ゲノムアセンブリ](https://www.yodosha.co.jp/jikkenigaku/keyword/2789.html)の状況`Status`と、ゲノムのサイズ`Size (Mb)`、GC含量`GC%`、遺伝子の数`Genes`、タンパク質の数`Proteins`を調べる。
...
[大腸菌](https://ja.wikipedia.org/wiki/大腸菌) "Escherichia coli" の[ゲノムアセンブリ](https://www.yodosha.co.jp/jikkenigaku/keyword/2789.html)の状況`Status`を確認する:  
    ...
    grep "Escherichia coli" prokaryotes.txt | cut -f16 | sort | uniq -c

大腸菌の完全ゲノム（"Complete Genome"）の1,7,8,12,13列目の情報をファイル出力する:  

    (grep "^#" prokaryotes.txt; grep "Escherichia coli.*Complete" prokaryotes.txt) | cut -f1,7,8,12,13 > output.ecoli.txt
