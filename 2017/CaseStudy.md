...
列番号を付けて出力する: 
    ...
    cd ~/projects/ncbiGenomeList/data/
    FILE="overview.txt" #FILE="eukaryotes.txt" #FILE="prokaryotes.txt" #FILE="viruses.txt"
...
で、8列目（`GC%`）の数値に対して統計値（平均値、最大値、最小値）を計算する: 

    # mean
    grep "Paramecium" eukaryotes.txt | cut -f8 | awk '{sum += $1} END {print sum/NR}'

    # max
    grep "Paramecium" eukaryotes.txt | cut -f8 | awk 'NR==1 {max=$1} {if($1 > max) max = $1} END {print max}'

    # min
    grep "Paramecium" eukaryotes.txt | cut -f8 | awk 'NR==1 {min=$1} {if($1 < min) min = $1} END {print min}'
