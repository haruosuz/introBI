...
`viruses.txt`ファイルの5列目（`Group`）または9列目（`Host`）をカウントする:  
...
列ラベル（`#`で始まる行）と、[ゾウリムシ属](https://ja.wikipedia.org/wiki/ゾウリムシ) "Paramecium" の生物名`#Organism/Name`、ゲノムのサイズ`Size (Mb)`とGC含量`GC%`を抽出する:  
