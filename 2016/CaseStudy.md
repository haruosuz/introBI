...
[バシラス属 "Bacillus"](https://ja.wikipedia.org/wiki/バシラス属) にマッチする行を抽出し、カウントする:  
...
[真核生物 "Eukaryota"](https://ja.wikipedia.org/wiki/真核生物) にマッチする行を抽出し、3,4列目（`Group, SubGroup`）をカウントする:  
...
ファイルの一行目（列ラベル）と[トラフグ属 "Takifugu"](https://ja.wikipedia.org/wiki/トラフグ属)にマッチする行を抽出し、`output.txt`ファイルへ出力する:  
...
以下の通り、`Takifugu flavidus`と`Takifugu rubripes`の2種のゲノム・プロジェクトが登録され、`Kingdom`は*Eukaryota*に、`Group`は*Animals*に、`SubGroup`は*Fishes*に[分類](https://ja.wikipedia.org/wiki/生物の分類#.E7.8F.BE.E5.9C.A8.E3.81.AE.E7.94.9F.E7.89.A9.E5.88.86.E9.A1.9E)され、ゲノムのサイズ `Size (Mb)`は378.032～391.485の範囲を示した。 
...
#### eukaryotes
[真核生物](https://ja.wikipedia.org/wiki/真核生物)のゲノム・リスト（*eukaryotes.txt*）を用いる。
...
[大腸菌 "Escherichia coli"](https://ja.wikipedia.org/wiki/大腸菌) の[ゲノムアセンブリ](https://www.yodosha.co.jp/jikkenigaku/keyword/2789.html)の状況`Status`を確認する:  

    grep "Escherichia coli" prokaryotes.txt | cut -f19 | sort | uniq -c

大腸菌の完全ゲノム（Complete Genome）の情報`Size (Mb), GC%, Genes, Proteins`をファイル出力する:  
...
[ゲノムアセンブリ](https://www.yodosha.co.jp/jikkenigaku/keyword/2789.html)の状況`Status`が"Complete Genome"のデータを`grep`関数で抽出する:  
...
`Group`が[菌類 "Fungi"](https://ja.wikipedia.org/wiki/菌類) に属するデータを抽出する:  
