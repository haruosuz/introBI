...
以下の通り、`Takifugu flavidus`と`Takifugu rubripes`の2種のゲノム・プロジェクトが登録され、`Kingdom`は*Eukaryota*に、`Group`は*Proteobacteria*に、`SubGroup`は*Alphaproteobacteria*に[分類](https://ja.wikipedia.org/wiki/生物の分類#.E7.8F.BE.E5.9C.A8.E3.81.AE.E7.94.9F.E7.89.A9.E5.88.86.E9.A1.9E)され、ゲノムのサイズ `Size (Mb)`は378.032～391.485の範囲を示した。 
