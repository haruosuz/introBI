...
`less README`でファイルを開いて、`/`を押して、文字列`prokaryotes.txt`を入力（終了するには、*q*を押す）。
...
大腸菌の完全ゲノム(Complete Genome)配列の特徴 Size (Mb), GC%, Genes, Proteins を抽出する:  

    head -n 1 prokaryotes.txt | cut -f1,7-8,15-16 > output.ecoli.txt
    grep "Escherichia coli.*Complete Genome" prokaryotes.txt | cut -f1,7-8,15-16 >> output.ecoli.txt
...
データのインポート。`read.delim()`関数でタブ区切りファイル"prokaryotes.txt"を読み込む:  
...
[プロット](http://cse.naro.affrc.go.jp/takezawa/r-tips/r/48.html)
    ...
    par(mfcol=c(2,1), cex=0.9, mai = c(1.2, 2.1, 0.1, 0.1)) # c(bottom, left, top, right)
    grp <- "SubGroup"
